...
## 35. `recibidos/nivel_documento` deja de parsear XML — lee `raw_sat`, con estatus dedicado para el hueco de backfill
...
## 36. `baseline_universal.py`: dos pares de consultas redundantes fusionados — el cuello real es la latencia por consulta (900ms/query), paralelizar queda pendiente
...
(punto 35/`ajustes_desde_sat`); lo único que queda por exprimir es el
