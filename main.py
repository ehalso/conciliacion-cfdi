#!/usr/bin/env python3
"""Conciliación CFDI recibidos: SAT (raw_sat.cfdi_recibidos) vs MPRO
(Comprobante_Digital), ambos vía el bridge de solo lectura en ctunlinux.
...
def run(periodos: list[str], tolerancia: str = "1.00", salida: str | None = None, label: str | None = None):
    label = label or "-".join(periodos)
    salida = salida or f"output/conciliacion_{label}.xlsx"
    Path(salida).parent.mkdir(parents=True, exist_ok=True)

    print(f"[1/4] Extrayendo SAT recibidos periodos={periodos} ...")
    sat_df = extract_sat_recibidos(periodos=periodos)
    print(f"      {len(sat_df)} filas de raw_sat.cfdi_recibidos")

    if sat_df.empty:
        print("Sin datos SAT para ese/esos periodo(s), nada que conciliar.")
        return None, None

    print(f"[2/4] Buscando {sat_df['uuid'].nunique()} UUIDs en Comprobante_Digital (MPRO, .207) ...")
...
def main():
    ...
    run(periodos, tolerancia=args.tolerancia, salida=args.salida, label=args.label)
