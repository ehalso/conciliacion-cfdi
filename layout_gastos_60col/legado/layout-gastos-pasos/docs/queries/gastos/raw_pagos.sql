-- Filas crudas de pago (Pago_CXP), via CXP-match (documento + referencia
-- consolidada -- este UNION es logica relacional real, no "elegir uno",
-- se queda en SQL). Sin ROW_NUMBER, sin representante -- eso es Python
-- (reglas_negocio.resolver_pago).
WITH folios AS (
    SELECT
...
SELECT
    cd.FOLIO, cd.Cxp_Folio, pc.Pc_ID, pc.Fp_Cve_Forma_Pago,
    pc.Pc_Banco, pc.Pc_Cuenta_Bancaria, pc.Pc_Documento, pc.Pc_Tabla, pc.Pc_Importe
FROM cxp_docs cd
