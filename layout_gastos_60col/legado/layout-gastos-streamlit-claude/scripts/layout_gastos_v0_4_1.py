"""
Layout de gastos - v0.4.1: como v0.3 (atribucion exacta por documento via centro de
costo), pero reduce mucho mas el uso del metodo de respaldo ("folio_completo"). Se
encontro (ver
folio 01-0034739, drill-down real de MPRO) que la causa de la mayoria de los casos que
caian a respaldo en v0.3 NO era falta de centro de costo, sino que la poliza a veces
COMBINA 2+ documentos identicos del mismo centro de costo en 1 sola linea (ej. 2 rentas
de $42,767.65 cada una, mismo centro 000501, quedan en 1 linea de poliza de $85,535.30) --
eso rompe el emparejamiento POSICIONAL 1 a 1 de v0.3 (2 documentos vs 1 linea de poliza).

Fix: cuando el conteo de lineas no coincide entre Gasto_Registro_Control y Poliza_Detalle
para un (folio, centro_costo), pero AMBOS lados tienen datos, se reparte el total de la
poliza de ese grupo PROPORCIONALMENTE entre los documentos de Gasto_Registro_Control de
ese mismo grupo, segun lo que cada uno aporto (Grc_Importe). Sigue siendo una atribucion
exacta a nivel de grupo (folio+centro_costo) -- 327 de 6,867 grupos de enero 2026 (4.8%)
necesitaban este reparto proporcional en vez del emparejamiento posicional de v0.3.

Tres metodos de atribucion para "el resto" (blanco, VIAJE, ORDEN_COMPRA, CONTROL_COMBUSTIBLE),
de mas a menos preciso:
  1. centro_costo: emparejamiento posicional exacto (v0.3) -- mismo conteo de lineas en
     ambos lados dentro de (folio, centro_costo).
  2. centro_costo_proporcional: conteo distinto pero datos en ambos lados -- reparto
     proporcional por Grc_Importe (nuevo en v0.4).
  3. folio_completo: solo cuando falta un lado por completo (linea de poliza sin centro
     de costo -- la mayoria son de Abono, ~82% de esos casos -- o centro de costo que no
     aparece del todo en Gasto_Registro_Control para ese folio). Ya no hay forma de
     repartir, se pega al ULTIMO Grd_ID del folio como en v0.2/v0.3.

CONSUMO_INTERNO/GASTO_REGISTRO_NOMINA y GASTO_RECLASIFICACION: identico a v0.2/v0.3.

NOTA: esta version (v0.4.1) es la que existia antes del ajuste de reversiones contables
(ver v0.4.2 / docs/15_hito_v0_4_reversiones.md) -- todavia deja marcados como discrepancia
los folios de reclasificacion manual con Importe negativo (Cargo~Abono, neto~0), en vez
de resolverlos.

Uso: python3 scripts/layout_gastos_v0_4_1.py 2026-01-01 2026-02-01
"""
import sys
...
if __name__ == "__main__":
    ...
    out_path = f"output/layout_gastos_v0_4_1_{fecha_ini}_a_{fecha_fin_excl}.csv"
